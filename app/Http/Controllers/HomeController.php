...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $reservationsToday = Reservation::whereDate('schedule_date', now()->toDateString())->count();
        $totalPatientsUser = User::whereHas('role', function ($query) {
            $query->where('name', 'patient');
        })->count();

        // Monthly reservations: Map data for chart
        $monthlyReservations = Reservation::selectRaw('MONTH(schedule_date) as month, count(*) as count')
                                          ->groupBy('month')
